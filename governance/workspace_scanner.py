"""Workspace Task Scanner. Per P10.10: Workspace Task Capture, DECISION-003: TypeDB-First."""
import os
import re
import logging
...
def parse_todo_md(filepath: str) -> List[ParsedTask]:
    ...
    for row in rows:
        ...
        status, phase = normalize_status(row.get("status", "TODO")), row.get("phase", "")
        tasks.append(ParsedTask(task_id=task_id, name=name, status=status, phase=phase if phase else None,
                                description=row.get("description", name), gap_id=extract_gap_id(row),
                                source_file=rel_path, linked_rules=extract_linked_rules(row)))
...
def parse_phase_md(filepath: str) -> List[ParsedTask]:
    ...
    for row in rows:
        ...
        name = re.sub(r'\[([^\]]+)\]\([^)]+\)', r'\1', name)
        tasks.append(ParsedTask(task_id=task_id, name=name[:200] if name else task_id,
                                status=normalize_status(row.get("status", "TODO")), phase=phase, description=name,
                                gap_id=extract_gap_id(row), source_file=rel_path, linked_rules=extract_linked_rules(row)))
...
def parse_rd_md(filepath: str) -> List[ParsedTask]:
    ...
    for row in rows:
        ...
        if not task_id:
            continue
        name, notes = row.get("task", row.get("description", "")), row.get("notes", "")
        tasks.append(ParsedTask(task_id=task_id, name=name, status=normalize_status(row.get("status", "TODO")),
                                phase="R&D", description=f"{name}. {notes}".strip() if notes else name,
                                gap_id=extract_gap_id(row), source_file=rel_path, linked_rules=extract_linked_rules(row)))
...
def sync_tasks_to_typedb(tasks: List[ParsedTask]) -> Dict[str, int]:
    """Sync parsed tasks to TypeDB. Returns dict with counts: inserted, updated, skipped, errors."""
    ...
    for task in tasks:
        try:
            ...
            if existing:
                ...
                result = client.insert_task(task_id=task.task_id, name=task.name, status=task.status,
                                            phase=task.phase, body=task.description, gap_id=task.gap_id,
...
def capture_workspace_tasks() -> Dict[str, Any]:
    """Main entry point: Scan workspace and sync to TypeDB. Per P10.10."""
    tasks = scan_workspace()
    stats = sync_tasks_to_typedb(tasks)
    result = {"scanned": len(tasks), "inserted": stats["inserted"], "updated": stats["updated"],
              "skipped": stats["skipped"], "errors": stats["errors"],
              "sources": ["TODO.md", "docs/backlog/phases/PHASE-*.md", "docs/backlog/rd/RD-*.md"]}
