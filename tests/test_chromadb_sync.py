...
class TestCurrentSyncState:
    ...
    @pytest.mark.unit
    def test_schema_has_fifteen_rules(self):
        """TypeDB schema should have 15 rules defined (RULE-001 to RULE-015)."""
        data_file = GOVERNANCE_DIR / "data.tql"
        content = data_file.read_text()
        # Count rule entities
        rule_count = content.count("isa rule-entity")
        assert rule_count == 15, f"Expected 15 rules, found {rule_count}"
